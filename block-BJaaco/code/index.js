...
let sum = 0;
for (let collectors of collection){
  sum += collectors;
}
...
const sumEvens = (evenCollection) => {
  let sum = 0;
  for (let i = 0; i < evenCollection.length; i++) {
    if (i % 2 === 0){ 
    sum = sum + evenCollection[i];
    }
  }
  return sum;
}
 
console.log(sumEvens(evenCollection,"sum of even numbers"));
...
let oddSum = 0;
   for(let i = 0; i < oddCollection.length; i++){
      if(oddCollection[i]%2 !== 0){
      oddSum += oddCollection[i]
      }
   }
...
function times(numberTime, string) {
return numberTime + string;
